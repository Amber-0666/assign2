...
if (!$conn->select_db($db)) {
    ...
    // Create products table with IF NOT EXISTS
    ...
    if ($conn->query($sql)) {
        ...
        if ($row['count'] == 0) {
            // Insert all products
            $insert_sql = "INSERT INTO `products` (`name`, `category`, `price_mp`, `price_np`, `image`, `description`) VALUES 
                ".getAllProducts()."
            ";
            ...
            if ($conn->multi_query($insert_sql)) {
                ...
                echo "<p>✅ Database and products table created and populated successfully.</p>";
            } else {
                echo "<p>❌ Error inserting products: ".$conn->error."</p>";
            }
        }
    } else {
